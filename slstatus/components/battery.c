 ...
 */
    ...
    const char *
    battery_custom(const char *bat) /* custommade battery module */
    {
        // getting battery percentage
        char *bat_percentage;
        char *bat_state;
        char val[32]={};
        bat_percentage = battery_perc(bat);
        int bat_per = battery_perc_int(bat); /* to get battery % as number */

        // getting battery state
        bat_state = battery_state(bat);
        ...
        // appending battery state color
        strcat(val,"");
        if (strcmp(bat_state,"-") == 0){ /* discharging */
            if (bat_per <= 20)
                strcat(val,"");
            if (bat_per <= 5)
                strcat(val,"");
        }
        if (strcmp(bat_state,"+") == 0) /* charging */
            strcat(val,"");
        if (strcmp(bat_state,"o") == 0) /* full */
            strcat(val,"");

        // appending remaining characters
        strcat(val,"B:");
        strcat(val,bat_percentage);
        strcat(val,"%");
